...
def Q_root(T, Ngamma, ro, x, dt):
    ...
    ro *= 1e6
    sigma = 1.63e-22
    # sigma = pow((3 * Ngamma*1e6) / (4 * np.pi * alphaBH(T) * (ro*1e6) ** 2), 1. / 3.) / (Ngamma*1e6)
    # print("sigma =", sigma)
    m = (alphaBH(T) * betaH(T)) * ro ** 2 * dt
    n = ro - (alphaAH(T) + betaH(T)) * ro / (sigma * sc.c) - alphaBH(T) * ro ** 2 * dt - 2 * betaH(T) * ro ** 2 * dt
    p = -ro * (1 + x) - Ngamma - 1 / (sigma * sc.c * dt) + betaH(T) * ro / (sigma * sc.c) + betaH(T) * ro ** 2 * dt
    q = Ngamma + ro * x + x / (sigma * sc.c * dt)
    coef = [m, n, p, q]
    res = np.roots(coef)
    for i in res:
        if 1 > i > 0:
